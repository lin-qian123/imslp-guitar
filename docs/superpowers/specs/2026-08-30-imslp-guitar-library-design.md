...
- 日期：2026-08-30
- 状态：用户已批准总体方向；第一轮规格审查问题已修订，等待第二轮审查
...
`objects/` 中保存按内部 SHA-256 命名的唯一实体，同时保留 IMSLP SHA-1 作为来源校验值。分类下的 `scores/` 优先使用硬链接指向对象。硬链接不可用时只允许使用根目录内的相对符号链接；如果符号链接也不可用，分类页直接链接到 `objects/`，不得复制出第二个 PDF 实体。`storage_method` 属于每一条本地 `Membership` 路径，而不是全局文件记录。绝对符号链接和重复实体均不允许作为完成状态。
...
- 终态不可取得：`copyright_restricted`、`region_restricted`、`membership_delay`、`login_required`、`commercial_only`、`deleted`。
...
2. 保持 `for3guitars` 原树不变，在 `backups/for3guitars-pre-migration-<run-id>/` 创建只读可恢复副本或受验证的卷内克隆；备份包含原 metadata、logs、scripts 和全部 PDF。
3. 在 `_migration/<run-id>/For 3 guitars (arr)/` 构建新 schema、严格清单、对象引用和全部静态页面；旧数据只读转换，不原地修改。
4. 导入合格 PDF 到对象库并验证 SHA-256；6 份混合编制文件写入隔离审计清单，77 条工作级记录逐页复核。
5. 物化分类路径，验证新清单/对象/路径、PDF、翻译和 `file://` 链接。
6. 只有验收全部通过后，才把原 `for3guitars` 原子改名为带 run ID 的保留目录，并把 staging 原子切换为 `For 3 guitars (arr)`。任何一步失败都保留旧入口或可一条命令回滚。
...
- 相同 SHA-256 只对应一个对象实体；所有分类硬链接/相对符号链接或对象直链指向正确内容。
...
2. 冻结快照内所有允许取得的目标 PDF 已下载；版权/地区/会员/登录/删除等终态不可取得项和人工审计项有明确状态，不能混称成功下载。
